...
def check_card_sync(cc, mm, yy, cvv, proxy_url=None):
    ...
    *intent: stitch three protocols into one verdict — every step must land
    ...
    # Real decline code from Stripe — dc (decline_code) is the raw response
    # e.g. "generic_decline", "insufficient_funds", "authentication_required", etc.
    real = dc or code or msg or "unknown"

    if stripe_status == "succeeded":
        return {"status": "charged", "message": real if real != "unknown" else f"Charged ${AMOUNT}.00 USD",
                "reason": reason, "time": elapsed()}

    if stripe_status in ("requires_action", "requires_source_action") or \
       "authentication" in msg.lower():
        return {"status": "3ds", "message": real,
                "reason": reason, "time": elapsed()}

    if code == "card_declined":
        if dc == "insufficient_funds":
            return {"status": "approved", "message": real,
                    "reason": reason, "time": elapsed()}
        return {"status": "declined", "message": real,
                "reason": reason, "time": elapsed()}
