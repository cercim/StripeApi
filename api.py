    ...
    *intent: stitch three protocols into one verdict — every step must land
    ...
    if stripe_status in ("requires_action", "requires_source_action") or \
       "authentication" in msg.lower():
        real_msg = stripe_status or msg
        return {"status": "3ds", "message": real_msg,
                "reason": reason, "time": elapsed()}
...
async def check_card(cc, mm, yy, cvv, user_proxies=None, **_):
    ...
    if user_proxies:
        ...
        if src:
            raw     = src[0]
            raw_str = (
                (raw.get("url") or raw.get("raw")) if isinstance(raw, dict)
                else str(raw)
            )
            # _format_proxy is idempotent — safe to call whether the string is
            # already normalised (http://user:pass@host:port) or still raw
            proxy_url = _format_proxy(raw_str) if raw_str else None
