...
#  PATH A — Docker
# ══════════════════════════════════════════════════════════════════
# detect docker compose v2 plugin OR old docker-compose v1
_DC=""
...
if [[ -n "$_DC" ]]; then
  info "Docker detected → using $_DC"
  ...
  echo ""
  echo -e "  Logs    : ${CYAN}$_DC logs -f${RESET}"
  echo -e "  Stop    : ${CYAN}$_DC down${RESET}"
  echo -e "  Restart : ${CYAN}$_DC restart${RESET}"
  exit 0
fi
